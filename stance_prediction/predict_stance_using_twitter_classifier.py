...
a_fn_plt = 'output/twitter_classifier/stance_accuracy.png'
l_fn_plt = 'output/twitter_classifier/stance_loss.png'
a_fn_model = 'output/twitter_classifier/best_model_accuracy_decay_e-03.pth'
l_fn_model = 'output/twitter_classifier/best_model_loss_decay_e-03.pth'
...
def train(model, loss, optimizer, dataloader):
    ...
    train_loss = 0
    correct = 0
    for x, label, tw_label in dataloader:
        optimizer.zero_grad()
...
def validate(model, loss, dataloader):
    ...
    for x, label, tw_label in dataloader:
        output = model.forward(x.detach())
        # Using torchhe output of last element in sequence as predicted value
        result = output[:, -1:, :].to(torch.float).squeeze(1) + tw_label
        loss_calc = loss(result, label)
...
def main():
    ...
    train_op =\
        use_twitter_classifier.call_twitter_classifier('cleaned_data_train.json')
    val_op = \
        use_twitter_classifier.call_twitter_classifier('cleaned_data_val.json')

    # data_dump()
    dataset_train = utils.NewsDataset('cleaned_data_train.json', perform_NER=True,
                                      use_twitter_classifier=True, tw_labels=train_op)
    ...
    dataset_val = utils.NewsDataset('cleaned_data_val.json', perform_NER=True,
                                    use_twitter_classifier=True, tw_labels=val_op)
